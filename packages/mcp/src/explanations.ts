...
export const EXPLANATIONS: Readonly<Record<string, Explanation>> = {
  ...
  CAP001: {
    ...
    body:
      ...
      "clause. Under ?bs 0.2 this is a parse error, not a runtime trap — the compiler walks " +
      "every fn body and matches direct stdlib refs against the declared capability set.\n\n" +
      "The check is conservative: only direct references in the calling fn's own body. " +
      "Aliasing (`const t = time; t.now()`) is not detected, by design — the rule is " +
      "'the canonical names are tripwires.' Transitive call-graph propagation is future work; " +
      "for now the runtime $require still backs everything up.",
    example: {
