...
export const EXPLANATIONS: Readonly<Record<string, Explanation>> = {
  ...
  THR002: {
    ...
    body:
      ...
      "error parameter of the fn's return type. A fn returning `Result<T, ParseError>` that " +
      "calls `err(new ParseError(...))` does not need `throws { ParseError }` — the error is " +
      "signaled via the return value, not thrown. THR002 only fires when the constructed type " +
      "is absent from both the throws declaration and the Result error type.\n\n" +
