...
describe("SYN009: XMLHttpRequest() call detection", () => {
  ...
  it("does NOT fire on TS type-literal with XMLHttpRequest() as a non-first member (regression)", () => {
    const src =
      "?bs 0.7\n" +
      "fn test() -> void {\n" +
      "  type XhrLike = { foo: string; XMLHttpRequest() };\n" +
      "}\n";
    const result = compile(src);
    expect(result.warnings.some((w) => w.code === "SYN009")).toBe(false);
  });

  it("does NOT fire on TS type-literal with XMLHttpRequest() after comma-separated member", () => {
    const src =
      "?bs 0.7\n" +
      "fn test() -> void {\n" +
      "  type XhrLike = { send: () -> void, XMLHttpRequest() };\n" +
      "}\n";
    const result = compile(src);
    expect(result.warnings.some((w) => w.code === "SYN009")).toBe(false);
  });
