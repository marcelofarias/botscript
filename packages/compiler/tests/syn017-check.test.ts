...
describe("SYN017: Notification() construction detection", () => {
  ...
  it("does NOT fire on TS type-literal method signature preceded by semicolon: type T = { x: string; Notification() }", () => {
    const src =
      "?bs 0.7\n" +
      "fn setup() -> void {\n" +
  ...
  it("does NOT fire on TS type-literal method signature preceded by comma: type T = { x: string, Notification() }", () => {
    const src =
      "?bs 0.7\n" +
      "fn setup() -> void {\n" +
      "  type T = { x: string, Notification() }\n" +
