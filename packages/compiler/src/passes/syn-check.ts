 ...
 *
 *   SYN009  A `new XMLHttpRequest()`, `XMLHttpRequest()`, `new XMLHttpRequest` (no-parens),
 *           `new XMLHttpRequest<T>()`, or `XMLHttpRequest?.()` was detected in a fn body
 *           (?bs 0.7+). `XMLHttpRequest` is the legacy browser HTTP API and opens an HTTP
 *           connection invisible to CAP001 (which checks `http.*` member calls, not the XHR
 *           global). A fn that constructs an XHR has an undeclared network dependency.
 *           Excluded: member calls (`obj.XMLHttpRequest`), object/class method shorthands,
 *           and TypeScript method signatures (`{ XMLHttpRequest(url): T; }`). The `:`
 *           exclusion is guarded against ternary consequents. Generic `<T>` detection
 *           only when preceded by `new` (avoids false-positives on comparison expressions).
 *
