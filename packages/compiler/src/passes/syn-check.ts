    ...
    for (let i = bodyStart; i < decl.tokenEnd; i++) {
      ...
      switch (tok.text) {
        ...
        case "import": {
          ...
          // Detect `import . meta . env` (or `?.` variants at either dot).
          // All other `import.*` property accesses (import.meta.url, etc.) fall through
          // to the `continue` below and are excluded from SYN011.
          if (next11 && next11.kind === "punct" && next11.text === ".") {
            ...
            // All `import.X` forms (not import.meta.env): skip SYN011.
            continue;
          }
