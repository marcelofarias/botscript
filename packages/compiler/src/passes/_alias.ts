...
/**
 * Collect module-level `const <alias> = <stdlib_namespace>` bindings where the
 * RHS is in a non-trivial form (i.e., would NOT be tracked by collectStdlibAliases).
 * These are ALI001 candidates — the binding starts with a stdlib ident but the
 * form is unsound (member access, operator, call, etc.).
 */
export function collectAliasWarningCandidates(
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    // Determine if trivial (would be tracked) or non-trivial (ALI001 candidate).
    let afterIdx = afterStdlibIdx;
    while (
      afterIdx < tokens.length &&
