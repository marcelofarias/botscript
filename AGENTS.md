...
| SYN034 | (0.7+, warning) A fn body constructs `new MessageChannel()`, bare `MessageChannel()`, or `MessageChannel?.()`. `MessageChannel` creates a pair of transferable `MessagePort` objects — `port1` and `port2` — that can be passed to workers, iframes, or service workers via `postMessage(msg, [port])`, enabling cross-context and cross-origin communication invisible to the capability model. Unlike `BroadcastChannel` (SYN014, same-origin named bus), `MessageChannel` ports can cross origin boundaries once transferred. Detection: `MessageChannel` ident not preceded by `.`/`?.`, followed by `(` or `?.(`. Member calls, fn/function/function* declarations named `MessageChannel`, object method shorthands, TypeScript method signatures, `unsafe {}` blocks, and `unsafe "reason" fn` bodies are excluded. | Wrap in `unsafe "<reason>" { new MessageChannel() }` to make the escape hatch and its justification visible in the diff. |
