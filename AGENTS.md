...
| SYN020 | (0.7+, warning) A fn body calls `Date.now()`, `Date?.now()`, `Date.now?.()`, `new Date()` (no args), `new Date<T>()` (TypeScript generic, no args), or `Date()` / `Date?.()` (no args). These forms inject the current wall-clock time at runtime but are invisible to botscript's capability model: `uses { time }` covers `time.*` stdlib calls, not the `Date` global. A fn that calls these forms has an undeclared time dependency — callers cannot see it and tests cannot control the time value observed. Excluded: `new Date(timestamp)` / `new Date('str')` / `new Date(y,m,d,...)` (explicit args), `Date.parse(str)` / `Date.UTC(...)`, `obj.Date()` (member call), fn/function/function* declarations named `Date`. `unsafe {}` blocks and `unsafe "reason" fn` bodies are suppressed. | Pass `nowMs` as an explicit parameter so tests can control the time. Alternatively, use `time.now()` with `uses { time }`. If direct `Date` access is required, wrap in `unsafe "uses current time for <reason>" { Date.now() }`. |
