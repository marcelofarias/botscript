...
| SYN021 | (0.7+, warning) A fn body calls `performance.now()`, `performance?.now()`, `performance.now?.()`, or reads `performance.timeOrigin` / `performance?.timeOrigin`. `performance.now()` returns a high-resolution monotonic timestamp and `performance.timeOrigin` exposes the epoch of that clock — both inject ambient timing information invisible to botscript's capability model (`uses { time }` covers `time.*` stdlib calls, not the `performance` global). Excluded: `obj.performance.*` (member access on local binding), `performance.mark()` / `performance.measure()` (no ambient read), fn/function declarations named `performance`. `unsafe {}` blocks and `unsafe "reason" fn` bodies are suppressed. | Pass `nowMs` as an explicit parameter so tests can control the time. Alternatively, use `time.now()` with `uses { time }`. If direct `performance` access is required, wrap in `unsafe "uses performance.now for <reason>" { performance.now() }`. |
