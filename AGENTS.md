...
| SYN004 | (0.7+, warning) A fn body contains a `process.exit()` or `process.abort()` call. These terminate the entire worker process — co-located bots are killed, no Result error path is available to callers, and even a wrapping `try/catch` cannot intercept the exit. This is the most severe bypass of botscript's safety model. | Bubble the exit intent as a `Result<T, E>` return value and let the orchestrator decide; if process termination is genuinely required, wrap in `unsafe "reason" { process.exit(...) }`. |
