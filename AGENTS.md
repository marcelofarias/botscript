...
| ALI001 | (0.8+, warning) A module-level `const <name> = <expr>` starts with a stdlib namespace ident but the RHS is too non-trivial to alias-track (member access, operator, call, etc.). Static checks won't see `name` as a stdlib alias. Non-blocking. | Use a direct binding `const t = time` for alias tracking, or call the namespace directly. |
...
| ALI003 | (0.8+, warning) A module-level `const { … } = <stdlib>` object-destructuring extracts member references that no static check follows. The extracted idents are not recognized as stdlib aliases; cap/intent/uns checks will miss any calls through them. Non-blocking. | Use a direct namespace binding (`const t = time`) and call `t.method()` instead of destructuring. |
