...
| SYN001 | Duplicate fn header clause (e.g. two `reads { }` on the same fn, or two `intent:`), or a label inside `reads {}` / `writes {}` that is not a plain identifier. Fires at any version pin when the clause being duplicated is present (e.g. `intent:` is 0.7+, `reads`/`writes` are 0.8+). | Declare each header clause once; merge label lists rather than repeating the clause; use bare identifiers (not quoted strings) as labels. |
