...
| SYN025 | (0.7+, warning) A fn body accesses `document.cookie` or `document?.cookie`. `document.cookie` reads or writes the browser's ambient cookie store at runtime — a shared mutable state invisible to botscript's capability model. Cookie access crosses security boundaries (HttpOnly, SameSite, domain scope), making it especially hazardous in agent/bot contexts. Detection: `document` ident not preceded by `.`/`?.`, followed by `.` or `?.`, then `cookie`. `obj.document.cookie` member calls, `fn`/`function`/`function*` declarations named `document`, other `document.*` members, `unsafe {}` blocks, and `unsafe "reason" fn` bodies are excluded. | Pass the cookie string as an explicit fn parameter so callers and tests can control it. If direct cookie access is required, wrap in `unsafe "reads/writes cookie for <reason>" { document.cookie }`. |
