...
| SYN020 | (0.7+, warning) A fn body calls `Date.now()`, `Date?.now()`, `Date.now?.()`, `new Date()` (no args), `new Date<T>()` (TypeScript generic, no args), or `Date(...)` / `Date?.()` (bare call — JS ignores arguments and always returns the current date string). These forms inject the current wall-clock time at runtime but are invisible to botscript's capability model: `uses { time }` covers `time.*` stdlib calls, not the `Date` global. A fn that calls these forms has an undeclared time dependency — callers cannot see it and tests cannot control the time value observed. Excluded: `new Date(timestamp)` / `new Date('str')` / `new Date(y,m,d,...)` (explicit args to `new Date`), `Date.parse(str)` / `Date.UTC(...)`, `obj.Date()` (member call), fn/function/function* declarations named `Date`. `unsafe {}` blocks and `unsafe "reason" fn` bodies are suppressed. | Pass `nowMs` as an explicit parameter so tests can control the time. Alternatively, use `time.now()` with `uses { time }` (note: `time.now()` returns epoch ms, not a Date object). If direct `Date` access is required, wrap in `unsafe "uses current time for <reason>" { Date.now() }`. |
| SYN021 | (0.7+, warning) A fn body calls `performance.now()`, `performance?.now()`, `performance.now?.()`, or reads `performance.timeOrigin` / `performance?.timeOrigin`. `performance.now()` returns a high-resolution monotonic timestamp and `performance.timeOrigin` exposes the epoch of that clock — both inject ambient timing information invisible to botscript's capability model (`uses { time }` covers `time.*` stdlib calls, not the `performance` global). Excluded: `obj.performance.*` (member access on local binding), `performance.mark()` / `performance.measure()` (no ambient read), fn/function declarations named `performance`. `unsafe {}` blocks and `unsafe "reason" fn` bodies are suppressed. | Pass `nowMs` as an explicit parameter so tests can control the time (preferred). Note: `time.now()` is wall-clock epoch time and is NOT a semantic replacement for `performance.now()` (monotonic clock) — only use `time.now()` if current epoch time, not elapsed time, is what the fn needs. If direct `performance` access is required, wrap in `unsafe "uses performance.now for <reason>" { performance.now() }`. |
