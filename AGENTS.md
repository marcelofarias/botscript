...
| INT003 | (0.7+) A fn declares `intent: "idempotent"` but also has `uses { random }` or `uses { time }`. Both capabilities produce different values on each call, making the function non-idempotent. Other capabilities (net, fs) are idempotent-compatible. | Remove `random`/`time` from `uses {}`, or change the intent. |
