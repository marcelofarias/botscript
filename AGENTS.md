...
| SYN004 | (0.7+, warning) A fn body contains a `process.exit()`, `process.abort()`, or `process.exitCode = N` call or assignment. `process.exit()` / `process.abort()` terminate the entire worker process — co-located bots are killed, no Result error path is available to callers, and even a wrapping `try/catch` cannot intercept the exit. `process.exitCode = N` sets the exit code used on eventual termination, which has the same effect from the orchestrator's perspective. All three forms are the most severe bypass of botscript's safety model. | Bubble the exit intent as a `Result<T, E>` return value and let the orchestrator decide; if process termination is genuinely required, wrap in `unsafe "reason" { process.exit(...) }`. |
